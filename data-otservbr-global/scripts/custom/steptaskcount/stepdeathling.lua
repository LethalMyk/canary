...
function PvpStep.onStepIn(creature, item, position, toPosition, fromPosition)
    ...
    local checkDeathling = Storage.TaskPoints.Deathling
    if player:getStorageValue(checkDeathling) == 1 then
        return true
    end
    ...
    local Deathlinglevelup = player:getStorageValue(Storage.TaskPoints.DeathlingLevelUp)
    local levelresp = 400 + Deathlinglevelup
    ...
    player:setCapacity(newMaxCap)  
        ...
        player:setSpeed(newSpeed)
            ...
            player:setStorageValue(checkDeathling, 1)    -- Define que o jogador está no respawn
        
        
--[[                 -- Checa se o jogador está com a task do respawn ativa
            local Deathlingactivetask = Storage.TaskPoints.DeathlingAtiveTask
            if player:getStorageValue(Deathlingactivetask) <= 0 then      
            end
            ]]          
