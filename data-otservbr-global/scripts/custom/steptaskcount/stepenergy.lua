...
function PvpStep.onStepIn(creature, item, position, toPosition, fromPosition)
    ...
    local checkenergy = Storage.TaskPoints.Energy
    if player:getStorageValue(checkenergy) == 1 then
        return true
    end
    ...
    local energylevelup = player:getStorageValue(Storage.TaskPoints.EnergyLevelUp)
    local levelresp = 650 + energylevelup
    ...
    player:setCapacity(newMaxCap)  
        ...
        player:setSpeed(newSpeed)
            ...
            player:setStorageValue(checkenergy, 1)    -- Define que o jogador está no respawn
        
        
--[[                 -- Checa se o jogador está com a task do respawn ativa
            local energyactivetask = Storage.TaskPoints.EnergyAtiveTask
            if player:getStorageValue(energyactivetask) <= 0 then      
            end
            ]]          
