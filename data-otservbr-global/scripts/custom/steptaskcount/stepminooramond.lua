...
function PvpStep.onStepIn(creature, item, position, toPosition, fromPosition)
    ...
    local checkMinoOramond = Storage.TaskPoints.MinoOramond
    if player:getStorageValue(checkMinoOramond) == 1 then
        return true
    end
    ...
    local MinoOramondlevelup = player:getStorageValue(Storage.TaskPoints.MinoOramondLevelUp)
    local levelresp = 45 + MinoOramondlevelup
    ...
    player:setCapacity(newMaxCap)  
        ...
        player:setSpeed(newSpeed)
            ...
            player:setStorageValue(checkMinoOramond, 1)    -- Define que o jogador está no respawn
        
        
--[[                 -- Checa se o jogador está com a task do respawn ativa
            local MinoOramondactivetask = Storage.TaskPoints.MinoOramondAtiveTask
            if player:getStorageValue(MinoOramondactivetask) <= 0 then      
            end
            ]]          
