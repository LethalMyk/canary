...
function PvpStep.onStepIn(creature, item, position, toPosition, fromPosition)
    ...
    local checkHulkingBehe = Storage.TaskPoints.HulkingBehe
    if player:getStorageValue(checkHulkingBehe) == 1 then
        return true
    end
    ...
    local HulkingBehelevelup = player:getStorageValue(Storage.TaskPoints.HulkingBeheLevelUp)
    local levelresp = 900 + HulkingBehelevelup
    ...
    player:setCapacity(newMaxCap)  
        ...
        player:setSpeed(newSpeed)
            ...
            player:setStorageValue(checkHulkingBehe, 1)    -- Define que o jogador está no respawn
        
        
--[[                 -- Checa se o jogador está com a task do respawn ativa
            local HulkingBeheactivetask = Storage.TaskPoints.HulkingBeheAtiveTask
            if player:getStorageValue(HulkingBeheactivetask) <= 0 then      
            end
            ]]          
