...
function PvpStep.onStepIn(creature, item, position, toPosition, fromPosition)
    ...
    local checkRotten = Storage.TaskPoints.Rotten
    if player:getStorageValue(checkRotten) == 1 then
        return true
    end
    ...
    local Rottenlevelup = player:getStorageValue(Storage.TaskPoints.RottenLevelUp)
    local levelresp = 850 + Rottenlevelup
    ...
    player:setCapacity(newMaxCap)  
        ...
        player:setSpeed(newSpeed)
            ...
            player:setStorageValue(checkRotten, 1)    -- Define que o jogador está no respawn
        
        
--[[                 -- Checa se o jogador está com a task do respawn ativa
            local Rottenactivetask = Storage.TaskPoints.RottenAtiveTask
            if player:getStorageValue(Rottenactivetask) <= 0 then      
            end
            ]]          
