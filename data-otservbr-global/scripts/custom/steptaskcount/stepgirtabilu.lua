...
function PvpStep.onStepIn(creature, item, position, toPosition, fromPosition)
    ...
    local checkGirtabilu = Storage.TaskPoints.Girtabilu
    if player:getStorageValue(checkGirtabilu) == 1 then
        return true
    end
    ...
    local Girtabilulevelup = player:getStorageValue(Storage.TaskPoints.GirtabiluLevelUp)
    local levelresp = 600 + Girtabilulevelup
    ...
    player:setCapacity(newMaxCap)  
        ...
        player:setSpeed(newSpeed)
            ...
            player:setStorageValue(checkGirtabilu, 1)    -- Define que o jogador está no respawn
        
        
--[[                 -- Checa se o jogador está com a task do respawn ativa
            local Girtabiluactivetask = Storage.TaskPoints.GirtabiluAtiveTask
            if player:getStorageValue(Girtabiluactivetask) <= 0 then      
            end
            ]]          
