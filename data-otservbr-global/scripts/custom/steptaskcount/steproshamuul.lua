...
function PvpStep.onStepIn(creature, item, position, toPosition, fromPosition)
    ...
    local checkRoshamuul = Storage.TaskPoints.Roshamuul
    if player:getStorageValue(checkRoshamuul) == 1 then
        return true
    end
    ...
    local Roshamuullevelup = player:getStorageValue(Storage.TaskPoints.RoshamuulLevelUp)
    local levelresp = 500 + Roshamuullevelup
    ...
    player:setCapacity(newMaxCap)  
        ...
        player:setSpeed(newSpeed)
            ...
            player:setStorageValue(checkRoshamuul, 1)    -- Define que o jogador está no respawn
        
        
--[[                 -- Checa se o jogador está com a task do respawn ativa
            local Roshamuulactivetask = Storage.TaskPoints.RoshamuulAtiveTask
            if player:getStorageValue(Roshamuulactivetask) <= 0 then      
            end
            ]]          
