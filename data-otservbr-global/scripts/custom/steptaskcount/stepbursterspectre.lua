...
function PvpStep.onStepIn(creature, item, position, toPosition, fromPosition)
    ...
    local checkBursterSpectre = Storage.TaskPoints.BursterSpectre
    if player:getStorageValue(checkBursterSpectre) == 1 then
        return true
    end
    ...
    local BursterSpectrelevelup = player:getStorageValue(Storage.TaskPoints.BursterSpectreLevelUp)
    local levelresp = 300 + BursterSpectrelevelup
    ...
    player:setCapacity(newMaxCap)  
        ...
        player:setSpeed(newSpeed)
            ...
            player:setStorageValue(checkBursterSpectre, 1)    -- Define que o jogador está no respawn
        
        
--[[                 -- Checa se o jogador está com a task do respawn ativa
            local BursterSpectreactivetask = Storage.TaskPoints.BursterSpectreAtiveTask
            if player:getStorageValue(BursterSpectreactivetask) <= 0 then      
            end
            ]]          
