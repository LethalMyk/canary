...
function PvpStep.onStepIn(creature, item, position, toPosition, fromPosition)
    ...
    local checkCobra = Storage.TaskPoints.Cobra
    if player:getStorageValue(checkCobra) == 1 then
        return true
    end
    ...
    local Cobralevelup = player:getStorageValue(Storage.TaskPoints.CobraLevelUp)
    local levelresp = 400 + Cobralevelup
    ...
    player:setCapacity(newMaxCap)  
        ...
        player:setSpeed(newSpeed)
            ...
            player:setStorageValue(checkCobra, 1)    -- Define que o jogador está no respawn
        
        
--[[                 -- Checa se o jogador está com a task do respawn ativa
            local Cobraactivetask = Storage.TaskPoints.CobraAtiveTask
            if player:getStorageValue(Cobraactivetask) <= 0 then      
            end
            ]]          
