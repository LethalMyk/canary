...
function PvpStep.onStepIn(creature, item, position, toPosition, fromPosition)
    ...
    local checkHellspawn = Storage.TaskPoints.Hellspawn
    if player:getStorageValue(checkHellspawn) == 1 then
        return true
    end
    ...
    local Hellspawnlevelup = player:getStorageValue(Storage.TaskPoints.HellspawnLevelUp)
    local levelresp = 120 + Hellspawnlevelup
    ...
    player:setCapacity(newMaxCap)  
        ...
        player:setSpeed(newSpeed)
            ...
            player:setStorageValue(checkHellspawn, 1)    -- Define que o jogador está no respawn
        
        
--[[                 -- Checa se o jogador está com a task do respawn ativa
            local Hellspawnactivetask = Storage.TaskPoints.HellspawnAtiveTask
            if player:getStorageValue(Hellspawnactivetask) <= 0 then      
            end
            ]]          
