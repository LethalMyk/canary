...
function PvpStep.onStepIn(creature, item, position, toPosition, fromPosition)
    ...
    local checkGoanna = Storage.TaskPoints.Goanna
    if player:getStorageValue(checkGoanna) == 1 then
        return true
    end
    ...
    local Goannalevelup = player:getStorageValue(Storage.TaskPoints.GoannaLevelUp)
    local levelresp = 600 + Goannalevelup
    ...
    player:setCapacity(newMaxCap)  
        ...
        player:setSpeed(newSpeed)
            ...
            player:setStorageValue(checkGoanna, 1)    -- Define que o jogador está no respawn
        
        
--[[                 -- Checa se o jogador está com a task do respawn ativa
            local Goannaactivetask = Storage.TaskPoints.GoannaAtiveTask
            if player:getStorageValue(Goannaactivetask) <= 0 then      
            end
            ]]          
