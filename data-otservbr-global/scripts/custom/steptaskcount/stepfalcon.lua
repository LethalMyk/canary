...
function PvpStep.onStepIn(creature, item, position, toPosition, fromPosition)
    ...
    local checkFalcon = Storage.TaskPoints.Falcon
    if player:getStorageValue(checkFalcon) == 1 then
        return true
    end
    ...
    local Falconlevelup = player:getStorageValue(Storage.TaskPoints.FalconLevelUp)
    local levelresp = 400 + Falconlevelup
    ...
    player:setCapacity(newMaxCap)  
        ...
        player:setSpeed(newSpeed)
            ...
            player:setStorageValue(checkFalcon, 1)    -- Define que o jogador está no respawn
        
        
--[[                 -- Checa se o jogador está com a task do respawn ativa
            local Falconactivetask = Storage.TaskPoints.FalconAtiveTask
            if player:getStorageValue(Falconactivetask) <= 0 then      
            end
            ]]          
