...
function PvpStep.onStepIn(creature, item, position, toPosition, fromPosition)
    ...
    local checkHero = Storage.TaskPoints.Hero
    if player:getStorageValue(checkHero) == 1 then
        return true
    end
    ...
    local Herolevelup = player:getStorageValue(Storage.TaskPoints.HeroLevelUp)
    local levelresp = 75 + Herolevelup
    ...
    player:setCapacity(newMaxCap)  
        ...
        player:setSpeed(newSpeed)
            ...
            player:setStorageValue(checkHero, 1)    -- Define que o jogador está no respawn
        
        
--[[                 -- Checa se o jogador está com a task do respawn ativa
            local Heroactivetask = Storage.TaskPoints.HeroAtiveTask
            if player:getStorageValue(Heroactivetask) <= 0 then      
            end
            ]]          
