...
function PvpStep.onStepIn(creature, item, position, toPosition, fromPosition)
    ...
    local checkSkeletin = Storage.TaskPoints.Skeletin
    if player:getStorageValue(checkSkeletin) == 1 then
        return true
    end
    ...
    local Skeletinlevelup = player:getStorageValue(Storage.TaskPoints.SkeletinLevelUp)
    local levelresp = 400 + Skeletinlevelup
    ...
    player:setCapacity(newMaxCap)  
        ...
        player:setSpeed(newSpeed)
            ...
            player:setStorageValue(checkSkeletin, 1)    -- Define que o jogador está no respawn
        
        
--[[                 -- Checa se o jogador está com a task do respawn ativa
            local Skeletinactivetask = Storage.TaskPoints.SkeletinAtiveTask
            if player:getStorageValue(Skeletinactivetask) <= 0 then      
            end
            ]]          
