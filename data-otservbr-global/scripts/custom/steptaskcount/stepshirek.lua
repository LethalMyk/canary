...
function PvpStep.onStepIn(creature, item, position, toPosition, fromPosition)
    ...
    local checkShirek = Storage.TaskPoints.Shirek
    if player:getStorageValue(checkShirek) == 1 then
        return true
    end
    ...
    local Shireklevelup = player:getStorageValue(Storage.TaskPoints.ShirekLevelUp)
    local levelresp = 900 + Shireklevelup
    ...
    player:setCapacity(newMaxCap)  
        ...
        player:setSpeed(newSpeed)
            ...
            player:setStorageValue(checkShirek, 1)    -- Define que o jogador está no respawn
        
        
--[[                 -- Checa se o jogador está com a task do respawn ativa
            local Shirekactivetask = Storage.TaskPoints.ShirekAtiveTask
            if player:getStorageValue(Shirekactivetask) <= 0 then      
            end
            ]]          
