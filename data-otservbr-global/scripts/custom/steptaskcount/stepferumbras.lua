...
function PvpStep.onStepIn(creature, item, position, toPosition, fromPosition)
    ...
    local checkFerumbras = Storage.TaskPoints.Ferumbras
    if player:getStorageValue(checkFerumbras) == 1 then
        return true
    end
    ...
    local Ferumbraslevelup = player:getStorageValue(Storage.TaskPoints.FerumbrasLevelUp)
    local levelresp = 600 + Ferumbraslevelup
    ...
    player:setCapacity(newMaxCap)  
        ...
        player:setSpeed(newSpeed)
            ...
            player:setStorageValue(checkFerumbras, 1)    -- Define que o jogador está no respawn
        
        
--[[                 -- Checa se o jogador está com a task do respawn ativa
            local Ferumbrasactivetask = Storage.TaskPoints.FerumbrasAtiveTask
            if player:getStorageValue(Ferumbrasactivetask) <= 0 then      
            end
            ]]          
