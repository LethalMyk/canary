...
function PvpStep.onStepIn(creature, item, position, toPosition, fromPosition)
    ...
    local checkSphinx = Storage.TaskPoints.Sphinx
    if player:getStorageValue(checkSphinx) == 1 then
        return true
    end
    ...
    local Sphinxlevelup = player:getStorageValue(Storage.TaskPoints.SphinxLevelUp)
    local levelresp = 190 + Sphinxlevelup
    ...
    player:setCapacity(newMaxCap)  
        ...
        player:setSpeed(newSpeed)
            ...
            player:setStorageValue(checkSphinx, 1)    -- Define que o jogador está no respawn
        
        
--[[                 -- Checa se o jogador está com a task do respawn ativa
            local Sphinxactivetask = Storage.TaskPoints.SphinxAtiveTask
            if player:getStorageValue(Sphinxactivetask) <= 0 then      
            end
            ]]          
