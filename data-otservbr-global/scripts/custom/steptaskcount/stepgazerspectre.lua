...
function PvpStep.onStepIn(creature, item, position, toPosition, fromPosition)
    ...
    local checkGazerSpectre = Storage.TaskPoints.GazerSpectre
    if player:getStorageValue(checkGazerSpectre) == 1 then
        return true
    end
    ...
    local GazerSpectrelevelup = player:getStorageValue(Storage.TaskPoints.GazerSpectreLevelUp)
    local levelresp = 300 + GazerSpectrelevelup
    ...
    player:setCapacity(newMaxCap)  
        ...
        player:setSpeed(newSpeed)
            ...
            player:setStorageValue(checkGazerSpectre, 1)    -- Define que o jogador está no respawn
        
        
--[[                 -- Checa se o jogador está com a task do respawn ativa
            local GazerSpectreactivetask = Storage.TaskPoints.GazerSpectreAtiveTask
            if player:getStorageValue(GazerSpectreactivetask) <= 0 then      
            end
            ]]          
