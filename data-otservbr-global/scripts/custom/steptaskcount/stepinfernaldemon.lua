...
function PvpStep.onStepIn(creature, item, position, toPosition, fromPosition)
    ...
    local checkInfernalDemon = Storage.TaskPoints.InfernalDemon
    if player:getStorageValue(checkInfernalDemon) == 1 then
        return true
    end
    ...
    local InfernalDemonlevelup = player:getStorageValue(Storage.TaskPoints.InfernalDemonLevelUp)
    local levelresp = 850 + InfernalDemonlevelup
    ...
    player:setCapacity(newMaxCap)  
        ...
        player:setSpeed(newSpeed)
            ...
            player:setStorageValue(checkInfernalDemon, 1)    -- Define que o jogador está no respawn
        
        
--[[                 -- Checa se o jogador está com a task do respawn ativa
            local InfernalDemonactivetask = Storage.TaskPoints.InfernalDemonAtiveTask
            if player:getStorageValue(InfernalDemonactivetask) <= 0 then      
            end
            ]]          
