...
function PvpStep.onStepIn(creature, item, position, toPosition, fromPosition)
    ...
    local checkTrueAsura = Storage.TaskPoints.TrueAsura
    if player:getStorageValue(checkTrueAsura) == 1 then
        return true
    end
    ...
    local TrueAsuralevelup = player:getStorageValue(Storage.TaskPoints.TrueAsuraLevelUp)
    local levelresp = 600 + TrueAsuralevelup
    ...
    player:setCapacity(newMaxCap)  
        ...
        player:setSpeed(newSpeed)
            ...
            player:setStorageValue(checkTrueAsura, 1)    -- Define que o jogador está no respawn
        
        
--[[                 -- Checa se o jogador está com a task do respawn ativa
            local TrueAsuraactivetask = Storage.TaskPoints.TrueAsuraAtiveTask
            if player:getStorageValue(TrueAsuraactivetask) <= 0 then      
            end
            ]]          
