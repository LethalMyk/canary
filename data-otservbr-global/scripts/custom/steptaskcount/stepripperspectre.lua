...
function PvpStep.onStepIn(creature, item, position, toPosition, fromPosition)
    ...
    local checkRipperSpectre = Storage.TaskPoints.RipperSpectre
    if player:getStorageValue(checkRipperSpectre) == 1 then
        return true
    end
    ...
    local RipperSpectrelevelup = player:getStorageValue(Storage.TaskPoints.RipperSpectreLevelUp)
    local levelresp = 300 + RipperSpectrelevelup
    ...
    player:setCapacity(newMaxCap)  
        ...
        player:setSpeed(newSpeed)
            ...
            player:setStorageValue(checkRipperSpectre, 1)    -- Define que o jogador está no respawn
        
        
--[[                 -- Checa se o jogador está com a task do respawn ativa
            local RipperSpectreactivetask = Storage.TaskPoints.RipperSpectreAtiveTask
            if player:getStorageValue(RipperSpectreactivetask) <= 0 then      
            end
            ]]          
