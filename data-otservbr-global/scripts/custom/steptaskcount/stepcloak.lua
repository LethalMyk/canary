...
function PvpStep.onStepIn(creature, item, position, toPosition, fromPosition)
    ...
    local checkCloak = Storage.TaskPoints.Cloak
    if player:getStorageValue(checkCloak) == 1 then
        return true
    end
    ...
    local Cloaklevelup = player:getStorageValue(Storage.TaskPoints.CloakLevelUp)
    local levelresp = 850 + Cloaklevelup
    ...
    player:setCapacity(newMaxCap)  
        ...
        player:setSpeed(newSpeed)
            ...
            player:setStorageValue(checkCloak, 1)    -- Define que o jogador está no respawn
        
        
--[[                 -- Checa se o jogador está com a task do respawn ativa
            local Cloakactivetask = Storage.TaskPoints.CloakAtiveTask
            if player:getStorageValue(Cloakactivetask) <= 0 then      
            end
            ]]          
