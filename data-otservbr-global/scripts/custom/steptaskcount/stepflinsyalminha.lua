...
function PvpStep.onStepIn(creature, item, position, toPosition, fromPosition)
    ...
    local checkFlinsyAlminha = Storage.TaskPoints.FlinsyAlminha
    if player:getStorageValue(checkFlinsyAlminha) == 1 then
        return true
    end
    ...
    local FlinsyAlminhalevelup = player:getStorageValue(Storage.TaskPoints.FlinsyAlminhaLevelUp)
    local levelresp = 400 + FlinsyAlminhalevelup
    ...
    player:setCapacity(newMaxCap)  
        ...
        player:setSpeed(newSpeed)
            ...
            player:setStorageValue(checkFlinsyAlminha, 1)    -- Define que o jogador está no respawn
        
        
--[[                 -- Checa se o jogador está com a task do respawn ativa
            local FlinsyAlminhaactivetask = Storage.TaskPoints.FlinsyAlminhaAtiveTask
            if player:getStorageValue(FlinsyAlminhaactivetask) <= 0 then      
            end
            ]]          
