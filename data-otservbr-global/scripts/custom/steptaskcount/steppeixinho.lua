...
function PvpStep.onStepIn(creature, item, position, toPosition, fromPosition)
    ...
    local checkPeixinho = Storage.TaskPoints.Peixinho
    if player:getStorageValue(checkPeixinho) == 1 then
        return true
    end
    ...
    local Peixinholevelup = player:getStorageValue(Storage.TaskPoints.PeixinhoLevelUp)
    local levelresp = 850 + Peixinholevelup
    ...
    player:setCapacity(newMaxCap)  
        ...
        player:setSpeed(newSpeed)
            ...
            player:setStorageValue(checkPeixinho, 1)    -- Define que o jogador está no respawn
        
        
--[[                 -- Checa se o jogador está com a task do respawn ativa
            local Peixinhoactivetask = Storage.TaskPoints.PeixinhoAtiveTask
            if player:getStorageValue(Peixinhoactivetask) <= 0 then      
            end
            ]]          
