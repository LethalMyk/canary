...
function PvpStep.onStepIn(creature, item, position, toPosition, fromPosition)
    ...
    local checkElfFogo = Storage.TaskPoints.ElfFogo
    if player:getStorageValue(checkElfFogo) == 1 then
        return true
    end
    ...
    local ElfFogolevelup = player:getStorageValue(Storage.TaskPoints.ElfFogoLevelUp)
    local levelresp = 300 + ElfFogolevelup
    ...
    player:setCapacity(newMaxCap)  
        ...
        player:setSpeed(newSpeed)
            ...
            player:setStorageValue(checkElfFogo, 1)    -- Define que o jogador está no respawn
        
        
--[[                 -- Checa se o jogador está com a task do respawn ativa
            local ElfFogoactivetask = Storage.TaskPoints.ElfFogoAtiveTask
            if player:getStorageValue(ElfFogoactivetask) <= 0 then      
            end
            ]]          
