...
function PvpStep.onStepIn(creature, item, position, toPosition, fromPosition)
    ...
    local checkBueried = Storage.TaskPoints.Bueried
    if player:getStorageValue(checkBueried) == 1 then
        return true
    end
    ...
    local Bueriedlevelup = player:getStorageValue(Storage.TaskPoints.BueriedLevelUp)
    local levelresp = 400 + Bueriedlevelup
    ...
    player:setCapacity(newMaxCap)  
        ...
        player:setSpeed(newSpeed)
            ...
            player:setStorageValue(checkBueried, 1)    -- Define que o jogador está no respawn
        
        
--[[                 -- Checa se o jogador está com a task do respawn ativa
            local Bueriedactivetask = Storage.TaskPoints.BueriedAtiveTask
            if player:getStorageValue(Bueriedactivetask) <= 0 then      
            end
            ]]          
