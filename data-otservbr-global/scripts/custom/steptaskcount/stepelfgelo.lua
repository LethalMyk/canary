...
function PvpStep.onStepIn(creature, item, position, toPosition, fromPosition)
    ...
    local checkElfGelo = Storage.TaskPoints.ElfGelo
    if player:getStorageValue(checkElfGelo) == 1 then
        return true
    end
    ...
    local ElfGelolevelup = player:getStorageValue(Storage.TaskPoints.ElfGeloLevelUp)
    local levelresp = 300 + ElfGelolevelup
    ...
    player:setCapacity(newMaxCap)  
        ...
        player:setSpeed(newSpeed)
            ...
            player:setStorageValue(checkElfGelo, 1)    -- Define que o jogador está no respawn
        
        
--[[                 -- Checa se o jogador está com a task do respawn ativa
            local ElfGeloactivetask = Storage.TaskPoints.ElfGeloAtiveTask
            if player:getStorageValue(ElfGeloactivetask) <= 0 then      
            end
            ]]          
