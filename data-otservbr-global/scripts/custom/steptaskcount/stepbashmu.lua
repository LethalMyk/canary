...
function PvpStep.onStepIn(creature, item, position, toPosition, fromPosition)
    ...
    local checkBashmu = Storage.TaskPoints.Bashmu
    if player:getStorageValue(checkBashmu) == 1 then
        return true
    end
    ...
    local Bashmulevelup = player:getStorageValue(Storage.TaskPoints.BashmuLevelUp)
    local levelresp = 600 + Bashmulevelup
    ...
    player:setCapacity(newMaxCap)  
        ...
        player:setSpeed(newSpeed)
            ...
            player:setStorageValue(checkBashmu, 1)    -- Define que o jogador está no respawn
        
        
--[[                 -- Checa se o jogador está com a task do respawn ativa
            local Bashmuactivetask = Storage.TaskPoints.BashmuAtiveTask
            if player:getStorageValue(Bashmuactivetask) <= 0 then      
            end
            ]]          
