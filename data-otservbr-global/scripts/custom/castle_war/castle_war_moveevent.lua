...
function castlewar.onStepIn(creature, item, position, fromPosition)
	...
	if actionId == 38131 then
		...
		if not Castle.opened then
			player:sendTextMessage(MESSAGE_INFO_DESCR, "Castle is closed.")
		...
		if not guild then
			...
			player:sendTextMessage(MESSAGE_INFO_DESCR, "How did you get in here? It wasn't even supposed to be here.")
			return false
		end
...
function dominatecastle.onStepIn(creature, item, position, fromPosition)
   ...
   if not guildNameDominate then
	...
	player:setStorageValue(cooldownStorage, os.time() + cooldownDuration)  
   	player:sendTextMessage(MESSAGE_INFO_DESCR, string.format('Currently the dominant guild is: %s', guildNameDominate))
...
local player = creature:getPlayer()
	...
	local guild = player:getGuild()
	if not guild then	
		player:sendTextMessage(MESSAGE_INFO_DESCR, "You are not from the Guild that dominates the Castle.")
