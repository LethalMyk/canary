...
function AcessBossScarlett.onStepIn(creature, item, position, toPosition, fromPosition)
    ...
    if player:getStorageValue(Storage.SevenDays.Scarlett) >= 7 then
        player:teleportTo(toPosition)
        player:addItem(60902, 1)
        player:setStorageValue(Storage.SevenDays.Scarlett, storageBoss -7)
